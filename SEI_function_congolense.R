...
df_obs <- read.csv("C:\\Users\\davry\\OneDrive\\Desktop\\mmed\\MMED Project\\MMED_C_CI.csv") 
...
optim.sann_C <- optim(par = guess.params,
                    fn = objFXN,
                    method = "SANN",
...
optim.final_C <- optim(par = optim.sann_C$par,
                     fn = objFXN,
                     method = "Nelder-Mead",
...
MLEfits_C <- optim.final_C$par
exp(unname(MLEfits_C))  

# Plot --------------------------------------------------------------------

fitted_df <- simEpidemic(init, tseq = tseqMonth, parms = subsParms(MLEfits_C, disease_params())) %>%
...
T.congolense_fitted_SEI_model_MLE <- ggplot(df_plot, aes(x = time)) +
  ...
  labs(title = "T.congolense fitted SEI model (MLE)",
...
ggsave("C:\\Users\\davry\\OneDrive\\Desktop\\mmed\\MMED Project\\graphs final\\T.congolense_fitted_SEI_model_MLE.png", 
       T.congolense_fitted_SEI_model_MLE, 
       device = "png", 
...
fisherInfMatrix_C <- solve(optim.final_C$hessian)  

ell_C <- ellipse(fisherInfMatrix_C, centre = MLEfits_C, level = 0.95)
ell_exp_C <- exp(ell_C)
ell_exp_C

lambda_hat_C <- exp(MLEfits_C['log_lambda'])
gamma_hat_C <- exp(MLEfits_C['log_gamma'])
gamma_hat_C
lambda_hat_C
...
plot(1, 1, type = 'n', log = 'xy',
     ...
     main = "MLE and 95% Confidence Region for Model Parameters\nfor SEI (T. congolense)", bty = "n")

points(lambda_hat_C, gamma_hat_C, pch = 16, cex = 1, col = 'blue')
lines(ell_exp_C, col = 'blue')
