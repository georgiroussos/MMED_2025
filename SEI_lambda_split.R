...
# Parameters --------------------------------------------------------------
disease_params <- function(lambda_null= 0.1,
                           lambda = 0.03,
                           gamma = 0.26,
                           mu = 0.0242,
                           phi = exp(-0.0242 * 9)) {
...
SImod <- function(tt, yy, parms) with(c(parms, as.list(yy)), {
  N <- S + E + I
  deriv <- rep(NA, 3)
  deriv[1] <- ifelse(tt == 0,-phi * lambda_null * S, -phi*lambda*S)
  deriv[2] <- ifelse(tt == 0, phi * (lambda_null * S - gamma * E),phi * (lambda * S - gamma * E))
  deriv[3] <- phi * gamma * E
  return(list(deriv))
})
...
guess.params <- c(log_lambda_null = log(0.1), log_lambda = log(0.03), log_gamma = log(0.3))
...
lambda_null_hat <- exp(MLEfits['log_lambda_null'])
gamma_hat
lambda_hat
lambda_null_hat
...
lambda_null_hat <-exp(log_lambda_null_hat)
lambda_hat 
gamma_hat 
lambda_null_hat
...
log_likelihood <- -nllikelihood(parms = disease_params(lambda = lambda_hat  , gamma = gamma_hat, lambda_null = lambda_null_hat), obsDat = df_obs)
